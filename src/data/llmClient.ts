...
export async function* postRenderResult(sessionId: string, stepId: number, ok: boolean, error: string, frame: string = "", nonce: string = "", verification?: VerificationReport): AsyncGenerator<ChatEvent> {
  // frame: ok=true 时可选,最后一帧 base64(视觉检查用);为空字符串则不带
  const body: Record<string, unknown> = { session_id: sessionId, step_id: stepId, ok, error, nonce, verification: verification && { ...verification, frame: "" } };
