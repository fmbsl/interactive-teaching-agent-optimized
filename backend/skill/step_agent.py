...
def _run_vision_check(frame_path: str, sid: str, step_id: int) -> str:
    ...
    try:
        ...
        # 教学意图作为上下文(帮助判断"画面是否符合该步要讲的内容")
        intent = _DRAFTS.get((sid, step_id), {}).get("intent", "")
        prompt = (
            "这是 manim 教学动画的最后一帧截图。请简短描述画面实际显示了什么,并检查:\n"
            "① 是否有文字/标签互相重叠、或文字压在图形上不可读;\n"
            "② 画面是否像是一个有意义的动画结果(若该步教学意图明确,据此判断画面是否体现);\n"
            f"③ 有无明显显示问题(文字超出画布、颜色与背景混淆不可见、画面空白无内容等)。\n"
            f"该步教学意图:{intent or '(未设)'}\n"
            "用 2-4 句中文回答,直接给结论。"
        )
