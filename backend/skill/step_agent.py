...
公式和正文优先使用 fontSize 20–32（核心公式建议 24–32），但这不是强制门槛。layout.place 会在区域不足时自动缩放，默认下限 12；确需更紧凑可用 `await layout.place(obj,'formula',{{minFontSize:10}})`，或用 `{{autoFit:false}}` 禁止自动缩放。长推导优先拆成多页，而不是一次塞满。多项公式组过宽时 place 会先按可见字形自动分行，不会删除项。
VGroup 经 arrange/place 定位后，必须把这个组作为整体 scene.add/Create/FadeIn；不要再单独 scene.add/play 组内子对象，否则 manim-web 会重新挂载子对象并丢失组变换。需要分步出现时，每一步使用独立、已定位的 VGroup。
